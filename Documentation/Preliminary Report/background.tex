...
Methods are impure so may have side effects on input parameters or other aspects of the program \cite{WhileyLang}. Functions, methods and other compound data types are passed by value meaning they are a copy of the original variable when they are passed in an argument to a function or method \cite{WhileyPlatform}.


A key component of Whiley is the automatic verifying compiler
...
Specifications can also be written as an invariant on user defined types (nominal type) or as an invariant on a loop using \texttt{where} clauses \cite{WhileyPlatform}. 
